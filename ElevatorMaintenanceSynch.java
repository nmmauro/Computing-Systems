...
public class ElevatorMaintenanceSynch extends Thread {
  ...
  public static ElevatorMaintenanceSynch es;
  public static int r;
  public static int cleaned;
  public static int last;
  public static Semaphore mMan = new Semaphore(1, false);
  public static boolean lock;
  
  public ElevatorMaintenanceSynch(int a, int b) {
    id = a;
    occupant = b;
  }
      ...
      if (call[current] && k > occupant) {
        if (available > k - occupant) {
          ...
          if(available == k - occupant) {
            ...
            occupant = 3;
          } else if (totalPeople[current] < k - occupant) {
  ...
  public static void semCheck() {
    ...
    last = Math.abs(checker - current) + last;
    System.out.println("Floors since maintenance: " + last);
  }
  
  public static void mMan() {
    System.out.println("Elevator needs cleaning");
    if (current == 0) {
      lock = false;
      System.out.println("Maintenance man requested");
      if (occupant == 0) {
        try {
          mMan.acquire();
        } catch (InterruptedException e) {}
        ...
        mMan.release();
        System.out.println("Elevator is now clean");
        last = 0;
      }
    }
  }
  ...
  public static void main(String[] args) {
    ...
    es = new ElevatorMaintenanceSynch(0, 0);
    es.start();
  }
}